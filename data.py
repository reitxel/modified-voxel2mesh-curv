...
from curv_loss import curv_from_cotcurv_laplacian, point_weigths_from_curvature
...
# takes a volume tensor as input and returns the outer surface of the voxel representation
def sample_outer_surface_in_voxel(volume): 
    ...
    b = F.max_pool3d(volume[None,None].float(), kernel_size=(1,3, 1), stride=1, padding=(0, 1, 0))[0]
    c = F.max_pool3d(volume[None,None].float(), kernel_size=(1,1,3), stride=1, padding=(0, 0, 1))[0] 
    border, _ = torch.max(torch.cat([a,b,c],dim=0),dim=0) # element-wise multiplication 
    surface = border - volume.float()
    return surface.long()
...
def voxel2mesh(volume, gap, shape):
    ...
    :return:
    '''
    # marching curves algorithm to find  surfaces in 3d volumetric data, Lewiner 
    # approach resolves ambiguities and guarantees topologically correct results 
    ...
    try: 
        ...
        #vertices_mc, faces_mc, _, _ = measure.marching_cubes(volume.cpu().data.numpy(), 0, step_size=gap, allow_degenerate=False, method = 'lewiner')
        vertices_mc, faces_mc, _, _ = measure.marching_cubes_lewiner(volume.cpu().data.numpy(), 0, step_size=gap, allow_degenerate=False)
...
def get_item(item, mode, config):
    ...
    if mode == DataModes.TRAINING:  # if training do augmentation
        ...
        theta = theta_rotate @ theta_shift @ theta_scale # matrix multiplication between the different transformations
...
#         if mode != DataModes.TRAINING:
        ...
        curvatures_gt = curv_from_cotcurv_laplacian(vertices_mc, faces_mc)

        vertices_mc_all += [vertices_mc]
        faces_mc_all += [faces_mc]
        curvs_all += [curvatures_gt]
       
        y_outer = sample_outer_surface_in_voxel((y==i).long()) 
        surface_points = torch.nonzero(y_outer, as_tuple=False) # added as_tuple
        ...
        # use available curvature elements as the point count for the surface sampling
        num_available_surface_points = surface_points_normalized.shape[0]
        num_curvatures = curvatures_gt.nelement()
        # Ensure point_count does not exceed the number of available surface points
        point_count = min(num_available_surface_points, 3000, num_curvatures)      
